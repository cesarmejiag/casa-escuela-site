...
const BackgroundColor = ({cSrcD, cSrcM, cColor,cHeight,children,cPosition}) => {
  const size = ScreenSize();
  const position = cPosition === "bottom" ? `${styles.bottom}` : `${styles.top}`;
  ...
  return (
    <div className={styles.wrapperBackground}>
      <div className={`${styles.colorElement} ${position}`} style={styleBack}>
      </div>
      {children}
    </div>
  )
